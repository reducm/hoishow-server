...
module Alipay
  module Notify
    def self.verify?(options)
      Sign.md5_verify?(options, AlipaySetting["v2_md5_key"]) 
    end
    ...
    def self.v2_app_verify?(options)
     sign = options.delete("sign")
     sign_type = options["sec_id"] || options.delete("sign_type")
     Sign.rsa_verify?(Alipay::Utils.hash_sort_to_string(options), sign, AlipaySetting["v2_alipay_rsa_pub_key"])
    end 
